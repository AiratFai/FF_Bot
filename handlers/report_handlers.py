...
from keyboards import report_kb, cat_kb, exp_report_cat_kb, inc_report_cat_kb, in_cat_kb, exit_kb
...
async def in_step1(message: types.Message, state: FSMContext):
    ...
    await message.reply(text, reply_markup=inc_report_cat_kb)
    await state.set_state(FSMIncomeReport.rep_period)


async def in_step2_1(message: types.Message, state: FSMContext):
    """Ловим ответ 1"""
...
async def in_step2_2(message: types.Message, state: FSMContext):
    """Ловим ответ 3"""
    await state.update_data(rep_period=message.text)
    data = await state.get_data()
    text = await create_income_report_text(data, None)
...
async def exp_step1(message: types.Message, state: FSMContext):
    """Начало диалога загрузки нового отчета"""
    text = 'Выбери период отчета'
    await message.reply(text, reply_markup=exp_report_cat_kb)
...
async def exp_step2_2(message: types.Message, state: FSMContext):
    ...
    await state.update_data(rep_period=message.text)
    data = await state.get_data()
    text = await create_expense_report_text(data, None)
    await message.reply(text)
    await bot.send_message(message.from_user.id, 'Продолжить работу?', reply_markup=exit_kb)
    await state.clear()
...
async def exp_step3(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    query = await orm.get_expense_report(data)
    text = await create_expense_report_text(data, query)
    await message.reply(text)
    await bot.send_message(message.from_user.id, 'Продолжить работу?', reply_markup=exit_kb)
    await state.clear()
...
def register_report_handlers(dp: Dispatcher):
    ...
    dp.message.register(in_step2_1, FSMIncomeReport.rep_period, lambda x: x.text in ['Текущий месяц', 'Текущий год'])
    dp.message.register(in_step2_2, FSMIncomeReport.rep_period, lambda x: x.text == 'Все доходы')
    dp.message.register(in_step3, FSMIncomeReport.rep_category)
    dp.message.register(exp_step1, Text(text='Отчеты по расходам'))
    dp.message.register(exp_step2_1, FSMIExpenseReport.exp_rep_period,
                        lambda x: x.text in ['Текущий месяц', 'Текущий год'])
    dp.message.register(exp_step2_2, FSMIExpenseReport.exp_rep_period, lambda x: x.text == 'Все расходы')
